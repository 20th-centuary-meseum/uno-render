...
void DecodeSprite(byte image_byte, short x_pos, short y_pos)
{
    ...
    if (x_pos < -8) // 2개 짤림
    {
        ...
        {
            TV.screen[tmp_y + i * MAX_X / 8] |= *(image_binary + i * 2 + 1) << -8 - x_pos;
        }
    }
    else if (x_pos < 0) // 1개 짤림
    {
        for (int i = y_minus; i < TILE_SIZE - y_plus; i++)
        {
            TV.screen[tmp_y + i * MAX_X / 8] |= *(image_binary + i * 2) << -x_pos | *(image_binary + i * 2 + 1) >> (8 + x_pos);
            TV.screen[tmp_y + i * MAX_X / 8 + 1] |= *(image_binary + i * 2 + 1) << -x_pos;
        }
    }
    else if (x_pos + 8 >= MAX_X) // 2개 짤림
    {
        for (int i = y_minus; i < TILE_SIZE - y_plus; i++)
        {
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8] |= *(image_binary + i * 2) >> shift_length;
        }
    }
    else if (x_pos + 16 >= MAX_X) // 1개 짤림
    {
        ...
        {
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8] |= *(image_binary + i * 2) >> shift_length;
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8 + 1] |= *(image_binary + i * 2) << (8 - shift_length) | *(image_binary + i * 2 + 1) >> shift_length;
        }
    }
    else // 안 짤림.
    {
        for (int i = y_minus; i < TILE_SIZE - y_plus; i++)
        {
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8] |= *(image_binary + i * 2) >> shift_length;
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8 + 1] |= *(image_binary + i * 2) << (8 - shift_length) | *(image_binary + i * 2 + 1) >> shift_length;
            TV.screen[tmp_y + x_pos / 8 + i * MAX_X / 8 + 2] |= *(image_binary + i * 2 + 1) << (8 - shift_length);
        }
    }
}
